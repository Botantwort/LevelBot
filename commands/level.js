                ...
                try {
                    axios.get(`https://discord.com/api/users/${mentionedMember.id}`, {
                        ...
                        .then((res) => {
                                ...
                                Informationen = "Zum ändern des Hintergrunds `" + client.prefix + "hintergrund` nutzen"
                                if (banner !== null) {
                                    img = `https://cdn.discordapp.com/banners/${mentionedMember.id}/${banner}.png?size=4096`
                                    Informationen = ""
                                } else {
                                    if (Hintergrund.Hintergrund !== null) {
                                        if (Hintergrund.Hintergrund !== "default") {
                                        img = Hintergrund.Hintergrund
                                        Informationen = ""
                                    }
                                }
                            }
                            if (Hintergrund.Hintergrund == "default") Informationen = ""
                            ...
                            Anzahllevel = target.level; AnzahlXP = target.xp - Levels.xpFor(target.level); RequiredXP = Levels.xpFor(target.level + 1) - Levels.xpFor(target.level); Platzierung = target.position; PlatzierungName = "RANK";
                            if (mentionedMember.user.bot) {
